...
jQuery( function( $ ) {
	...
	//--->  minimum order amount hack
      $.ajax({
        url: document.location.origin + wc_checkout_params.ajax_url,
        data: { 
          action: "minimum_order_amount" ,
          data: $("#select2-billing_country-container").text(),
        },
        type: "post",
        success: function (response) {
          
          var check_boolean = response.includes("true");
          ...
          wc_checkout_form.xhr = $.ajax({
            ...
            success:	function( data ) {
              ...
              setTimeout(function() {
                if(check_boolean) {
                  $(".woocommerce-checkout-payment .button.alt").css("display", "none");
                } else {
                  $(".woocommerce-checkout-payment .button.alt").css("display", "block");
                }
              }, 700);
